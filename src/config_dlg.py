        ...
        if self.current_network_cfg:
            ...
            try:
                ...
                if isinstance(dashd_conf, tuple) and len(dashd_conf) >= 3:
                    if not dashd_conf[0]:
                        self.infoMsg('Remore ZCore daemon seems to be shut down')
                    elif not dashd_conf[1]:
                        self.infoMsg('Could not find remote zcore.conf file')
                    else:
                        file = dashd_conf[2]
                        rpcuser = file.get('rpcuser', '')
                        rpcpassword = file.get('rpcpassword', '')
                        rpcport = file.get('rpcport', '27293' if self.local_config.is_testnet() else '17293')
                        modified = False
                        if rpcuser:
                        ...
                        if file.get('server', '1') == '0':
                            self.warnMsg("Remote zcore.conf parameter 'server' is set to '0', so RPC interface will "
                                         "not work.")
                        if not rpcuser:
                            self.warnMsg("Remote zcore.conf parameter 'rpcuser' is not set, so RPC interface will  "
                                         "not work.")
                        if not rpcpassword:
                            self.warnMsg("Remote zcore.conf parameter 'rpcpassword' is not set, so RPC interface will  "
                                         "not work.")
                    self.update_connection_details_ui()
                elif isinstance(dashd_conf, str):
                    self.warnMsg("Couldn't read remote zcore configuration file due the following error: " +
                                 dashd_conf)
                ssh.disconnect()
            except Exception as e:
                self.errorMsg(str(e))
